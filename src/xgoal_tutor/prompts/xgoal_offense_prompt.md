...
Here is a brief description of each Xgoal probability model feature:
...
The following passing simulation features are available:
...
You are also given a feature set from simulating whether a short move instead of shooting in the current position could lift xG. 
This movement simulation also accounts how the defenders and the goalkeeper respond.
Use these move simulation features in your explanation only if an additional movement would increase the xGoal probability.
...
- Always use meter as a measure for distance
- When referring to any player use the players’ real names exactly as given.
- When talking about influential factors use natural football language instead using the feature names directly, e.g. say "Shooter’s goal distance" instead of "dist_sb".

You are given attacking players, defenders, and the goalkeeper positions from a freeze-frame
(StatsBomb 120×80 grid; (0,0)=own goal-line left corner; (120,40)=centre of attacking goal This is a grid design by Statsbomb to locate the position on the field, but it does not refer to meters!).
...
Provide a concise explanation referencing the most influential factors using the Xgoal probability model feature, passing simulation features and movement simulation.
Always include key attacker(s), key defender(s) and the goalkeeper with their real name.
